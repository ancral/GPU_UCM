...
__global__ void addMatrixGPU(float *a, float *b, float *c, int N )
{
	...
	int j = threadIdx.y + blockDim.y *blockIdx.y;
		if (i <  N && j < N) a[i + j*N] = b[i+ j*N] + c[i+ j*N];
	
}

//No se realiza los computos correctos a partir de N >= 8627 en matrices de N*N
int main(int argc, char *argv[])
{
	...
	dim3 nBlocks;
	if (N%32==0)
		nBlocks = dim3(N/32,N/32);
	else
		nBlocks = dim3(N/32 +1,N/32 +1);
